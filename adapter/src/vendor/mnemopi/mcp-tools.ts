...
const STATS_SCHEMA = {
	type: "object",
	properties: { bank: { type: "string", description: "Memory bank to inspect." } },
	required: ["bank"],
} as const;
...
export const REMEMBER_SCHEMA = {
	...
	properties: {
		...
		source: { type: "string", description: "Source tag for this memory.", default: "mcp" },
		scope: {
			type: "string",
...
export const UPDATE_SCHEMA = {
	...
	required: ["memory_id", "content", "bank"],
} as const;
...
function createBeam(args: ToolArguments, bank = resolveBank(args)): BeamMemory {
	// OMP's native convention is session_id == bank. Keep the adapter on that
	// identity so native rows are visible to MCP recall/get/update calls.
	const sessionId = bank;
	return new BeamMemory({
		...
		channelId: sessionId,
		// External adapter writes must never trim old native working memories.
		config: { workingMemoryLimit: 0 },
	});
}
...
const ADAPTER_TOOL_NAMES = new Set([
	"mnemopi_remember",
	"mnemopi_recall",
	"mnemopi_get",
	"mnemopi_update",
...
export async function handleToolCall(name: string, args: ToolArguments = {}): Promise<ToolResult> {
	if (!ADAPTER_TOOL_NAMES.has(name)) throw new Error(`Unknown or unsupported adapter tool: ${name}`);
	const bank = args.bank;
	if (typeof bank !== "string" || bank.trim().length === 0) {
		throw new Error(`${name} requires an explicit non-empty bank argument`);
	}
